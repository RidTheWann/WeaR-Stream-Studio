...
try {
	...
	if (FetchAndVerifyFile("branches", "wear-stream-studio\\updates\\branches.json", WIN_BRANCHES_URL, &text)) {
	...
	text.clear();
	if (!FetchAndVerifyFile("manifest", "wear-stream-studio\\updates\\manifest.json", manifestUrl.c_str(), &text,
				extraHeaders)) {
		return;
	}
	...
	if (!FetchAndVerifyFile("updater", "wear-stream-studio\\updates\\updater.exe", WIN_UPDATER_URL, nullptr)) {
		return;
	}
	...
	BPtr<char> updateFilePath = GetAppConfigPathPtr("wear-stream-studio\\updates\\updater.exe");
