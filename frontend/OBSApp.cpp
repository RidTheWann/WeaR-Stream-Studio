...
static bool MakeUserDirs()
{
	...
	if (GetAppConfigPath(path, sizeof(path), "wear-stream-studio/basic") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
	...
	if (GetAppConfigPath(path, sizeof(path), "wear-stream-studio/logs") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
	...
	if (GetAppConfigPath(path, sizeof(path), "wear-stream-studio/profiler_data") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
...
#ifdef _WIN32
	if (GetAppConfigPath(path, sizeof(path), "wear-stream-studio/crashes") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
	...
	if (GetAppConfigPath(path, sizeof(path), "wear-stream-studio/updates") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
	...
	if (GetAppConfigPath(path, sizeof(path), "wear-stream-studio/plugin_config") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
...
constexpr std::string_view OBSProfileSubDirectory = "wear-stream-studio/basic/profiles";
constexpr std::string_view OBSScenesSubDirectory = "wear-stream-studio/basic/scenes";
constexpr std::string_view OBSPluginManagerSubDirectory = "wear-stream-studio/plugin_manager";
...
bool OBSApp::InitGlobalConfig()
{
	...
	int len = GetAppConfigPath(path, sizeof(path), "wear-stream-studio/global.ini");
	if (len <= 0) {
		return false;
	}
...
{
	const std::string userConfigFile = userConfigLocation.u8string() + "/wear-stream-studio/user.ini";
...
static constexpr string_view OBSGlobalIniPath = "/wear-stream-studio/global.ini";
static constexpr string_view OBSUserIniPath = "/wear-stream-studio/user.ini";
...
bool LoadBranchesFile(vector<UpdateBranch> &out)
{
	...
	BPtr<char> branchesFilePath = GetAppConfigPathPtr("wear-stream-studio/updates/branches.json");
...
static void move_basic_to_profiles(void)
{
	...
	if (GetAppConfigPath(path, 512, "wear-stream-studio/basic") <= 0) {
		return;
	}
	...
	const std::filesystem::path profilesPath =
		App()->userProfilesLocation / std::filesystem::u8path("wear-stream-studio/basic/profiles");
...
static void move_basic_to_scene_collections(void)
{
	...
	const std::filesystem::path sceneCollectionPath =
		App()->userScenesLocation / std::filesystem::u8path("wear-stream-studio/basic/scenes");
...
void OBSApp::uploadLastAppLog() const
{
	...
	basicWindow->UploadLog("wear-stream-studio/logs", GetLastLog(), OBS::LogFileType::LastAppLog);
}
...
void OBSApp::uploadCurrentAppLog() const
{
	...
	basicWindow->UploadLog("wear-stream-studio/logs", GetCurrentLog(), OBS::LogFileType::CurrentAppLog);
}
